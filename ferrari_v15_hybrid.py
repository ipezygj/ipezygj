""" Technical implementation for Hummingbot Gateway V2.1 - HYBRID V15 """
...
# 1. SETUP
print("🦈 FERRARI V15 - HYBRID (MOMENTUM + DIP SNIPER)")
print("-----------------------------------------------")
...
MAX_POSITIONS = 3       # 3 Positiota: 2 Trendille, 1 Dipille
LEVERAGE = 7
RISK_PCT_PER_TRADE = 0.28 # Turvallinen taso kolmelle kaupalle
TRAILING_GAP = 0.008
...
print(f"🌊 HUNTING GROUNDS: {len(TARGET_COINS)} Assets | HYBRID MODE ACTIVE")
...
while True:
    try:
        ...
        try:
            ...
            if "429" in str(e):
                print("\n⚠️ API 429. Cooling down 10s...", end="\r"); time.sleep(10); continue
        ...
        if open_slots > 0:
            ...
            print(f"{status_str} | 📡 Hybrid Scan...", end="\r")
            
            for coin in scan_batch:
                now_ms = int(time.time() * 1000)
                try:
                    c = info.candles_snapshot(coin, "5m", 20, now_ms) # 5min kynttilät
                    time.sleep(0.3)
                except: continue
                if not c: continue
                ...
                # --- STRATEGY 1: MOMENTUM (High Vol + Price UP) ---
                ...
                if vol_ratio > 2.5 and price_change > 0.002:
                    ...
                    if size > 0:
                        ...
                        log_trade(f"ENTRY {coin} (Momentum)")
                        peak_prices[coin] = last_price
                        time.sleep(2); break

                # --- STRATEGY 2: DIP SNIPER (Crash > 2% in 5mins) ---
                # Lasketaan muutos viimeisen 3 kynttilän aikana (15min)
                start_price = closes[-4] # 15min sitten
                drop_pct = (last_price - start_price) / start_price
                
                # Jos hinta on pudonnut yli 2.5% JA alkaa toipua (pieni vihreä kynttilä)
                if drop_pct < -0.025 and price_change > 0:
                    print(f"\n🩸 SCAVENGER BUY! {coin} Dumped {drop_pct*100:.1f}%")
                    size = round(((equity * RISK_PCT_PER_TRADE) * LEVERAGE) / last_price, 1)
                    if size > 0:
                        exch.market_open(coin, True, size, last_price, 0.01)
                        log_trade(f"ENTRY {coin} (Dip Sniper)")
                        peak_prices[coin] = last_price
                        time.sleep(2); break
